...
const Onboarding = () => {
  ...
  return (
    <SafeAreaView className="flex h-full items-center justify-center bg-[#8569bb]">
      {/* Header */}
      ...
      <Swiper
        ...
        activeDot={
          <View className="w-[32px] h-[4px] mx-1 bg-gray-700 rounded-full" />
        }
      ...
      >
        {onboarding.map((item) => (
          <View key={item.id} className="flex-1 items-center justify-center">
            ...
            <Text className="text-2xl font-JakartaBold mb-2 text-white">
              {item.title}
            </Text>
            <Text className="text-md text-gray-700 text-center px-5">
              {item.description}
            </Text>
          </View>
        ))}
      ...
      <TouchableOpacity
        onPress={handleNext}
        className="absolute bottom-10 bg-[#551aca] py-3 w-full px-8 rounded-full"
      >
